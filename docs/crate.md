...
PYTHONPATH=crate python3 -m ix_crate stemit --genre Rock --limit 100
PYTHONPATH=crate python3 -m ix_crate stemit --genre Rock --limit 100 --execute
PYTHONPATH=crate python3 -m ix_crate stemit --genre Rock --offset 100 --limit 100 --execute
...
3. **Daily crates.** `--playlists` writes stable names: `Played` (100 most recent `last_played`) and `Not Played But Should` / Neglected genres / Favorites / Random (12 each). Favorites-match uses genre + BPM ±6 + energy ±1. Random is last so it does not steal the match pool. Library acapella titles stay out of those picks. Skip the rewrite when the play fingerprint is unchanged. Quit Rekordbox for xml; quit Traktor for NML. Music.app only if it is open (flat `NPBS …` names).
4. **GENRES and ACAPELLAS.** Same `--playlists` write. Code: `crate/ix_crate/library_genres.py` and `crate/ix_crate/acapella_crates.py`. `MUSIC/GENRES/<Genre>` is every Apple Music file of that genre (shared iTunes library XML, `clean_genre`, file has to be on disk, `.m4p` skipped). `MUSIC/ACAPELLAS/<Genre>` is STEMIT vocal files (`Song_vocals.m4a`, `vocals.m4a`). Artist and title come from the filename with the role suffix removed, then the folder. Match the Apple Music library by artist + title, then by title with mix/feat tails removed. When several rows share the title, the folder artist picks; when every remaining row shares one genre, that genre is used. No Apple Music equivalent: MusicBrainz identifies the recording and the library match is tried again, then Shazam listens to the vocal and the library match is tried again. A Shazam or MusicBrainz genre is kept only when the song is still absent from Apple Music. There is no Untagged playlist. Open vocals are finished with `follow_unmatched()` in that module (MusicBrainz, then Shazam) and written back to `ACAPELLAS` only. A file already in the collection keeps that row; a new file gets a location row. Analyze stays in-app. Apple Music media is never written. The retired flat `Acapella` list and the older `Acapellas` folder are cleared on the next write.
5. **Quarterly git.** Live JSON is gitignored (~7 MB sitting list). Once a quarter: `--execute --snapshot` → `configs/quarterly/favorites-YYYYQn.json` (played + stats) + `play-patterns-YYYYQn.json`, then commit those two. First draft: **2026Q3**. Not nightly.
6. **Nightly agent (optional).** Example: `docs/examples/favorites-nightly.sh` + `ai.ixamal.crate-favorites.plist` (5:00). Not loaded until David asks.
